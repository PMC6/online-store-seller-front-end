<template>
  ...
  <div>
    ...
    <div style="display:flex;" class="products-order row">
      <button type="submit" class="btn col-sm-6" @click="getbyprice" >Price</button>
      <button type="submit" class="btn col-sm-6" @click="getbysales" >new products</button>
    </div>
    <!-- 商品展示列表 -->
  ...
  mounted() {
    ...
    Bus.$on("shopInfoLoad", function (data) {
      axios.get("/seller/product/num")
        .then(response=>{
          that.totalProduct = response.data.data;
        }).catch(()=>{
        that.totalProduct = 5;
      });
      that.pageChange();
